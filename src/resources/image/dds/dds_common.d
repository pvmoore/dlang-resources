module resources.image.dds.dds_common;
...
struct DDS_HEADER { static assert(DDS_HEADER.sizeof == 124);
    ...
    string toString() {
        return "DDS_HEADER(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)".format(
            size, flags, height, width, pitch, depth, mipMapLevels, format, toArray!DDSCAPS(caps), toArray!DDSCAPS2(caps2)
        );
    }
}
struct DDS_HEADER_DXT10 { static assert(DDS_HEADER_DXT10.sizeof == 20);
