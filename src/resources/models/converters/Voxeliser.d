module resources.models.converters.Voxeliser;
...
public:
     ...
     */
    void voxelise(uint voxelsDimension, bool fill) {
    ...
    Vox getVox() {
        throwIf(size == uint3(0), "voxelise() must be called before getVox()");
        Vox vox = new Vox;
        vox.size = size;
        vox.voxels = voxels;
        return vox;
    ...
    /**
     * Write the voxel data to a file with the following format:
     *  [0  : 4 bytes]     uint    = x size
     *  [4  : 4 bytes]     uint    = y size
     *  [8  : 4 bytes]     uint    = z size
     *  [12 : x*y*z bytes] ubyte[] = voxel array - 1 byte per voxel - compressed using BZip3
     */
    void write(string filename) {
        throwIf(size == uint3(0), "voxelise() must be called before write()");
        ...
        file.rawWrite(BZip3.compress(voxels, 10));
    }
private:
